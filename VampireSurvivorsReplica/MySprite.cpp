...
ComPtr<ID3D11ShaderResourceView> MySprite::GetSRV()
{
...
ComPtr<ID3D11ShaderResourceView> MySprite::GetSRV(UINT index)
{
...
void  MySprite::Load( std::wstring* iconList, UINT iCount )
{	
...
void  MySprite::Load(STRING_VECTOR& iconList)
{
...
void  MySprite::Load(std::wstring path, RECT_ARRAY& list)
{
...
void  MySprite::Load(std::wstring path, UINT xSize, UINT ySize)
{
	m_pTex = I_Tex.Load(path).get();
...
void   MySprite::Update(MySpriteData& data)
{
...
bool MySpriteManager::GameDataLoad(const TCHAR* pszLoad)
{
	TCHAR pBuffer[256] = { 0 };
	TCHAR pTemp[256] = { 0 };
...
bool MySpriteManager::Load(const TCHAR* pszLoad)
{
...
MySprite* MySpriteManager::GetPtr(W_STR name)
{
	auto iter = m_List.find(name);
	if (iter != m_List.end())
	{
		MySprite* pData = iter->second.get();
		return pData;
	}
	return nullptr;
}